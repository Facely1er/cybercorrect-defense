...
const AssessmentLayout = ({ toggleDarkMode, darkMode }: AssessmentLayoutProps) => {
  const { user } = useAuth();
          ...
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="hidden lg:flex items-center space-x-8">
              {mainNavItems.map(item => (
                <Link 
                  key={item.name}
                  to={item.path}
                  className={`flex items-center text-foreground hover:text-primary-teal transition-colors duration-200 px-3 py-2 text-sm font-medium ${
                    location.pathname === item.path ? 'text-primary-teal' : ''
                  }`}
                >
            ...
            <div className="flex items-center gap-3">
              ...
              <Link to={user ? "/app/dashboard" : "/login"}>
                <Button className="bg-primary-teal hover:bg-secondary-teal text-white rounded-full shadow-md hover:shadow-lg transition-all hover:-translate-y-1">
        ...
        <div className="container mx-auto px-4 py-6">
          ...
          {location.pathname !== '/assessment' && (
            <div className="mb-6">
              <Link 
                to="/assessment"
                className="inline-flex items-center text-muted-foreground hover:text-primary-teal transition-colors duration-200"
              >
